...
public class UserController {
	...
	public String processLoginForm(@RequestParam String email, @RequestParam String pass, Model map,
		...
		try {
			...
			// web browser sends next req --> http://host:port/ctx path/admin/main , method
			// : GET
	...
	public String userLogout(HttpSession session, Model map, HttpServletRequest rq, HttpServletResponse resp) {
		...
		map.addAttribute("user_details", session.getAttribute("user_details")); // session stored in model map to show
																				// name in logout pg
		// invalidate session
		session.invalidate(); // delete session
		// set refresh header : for auto redirection after delay
		resp.setHeader("refresh", "5;url=" + rq.getContextPath()); // it will keep logout pg for 5sec then refresh pg
																	// automatically and redirect to home pg
		return "/user/logout";
	}
